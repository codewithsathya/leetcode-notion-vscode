...
class LeetCodeManager extends EventEmitter {
    ...
    public async signIn(context: ExtensionContext): Promise<void> {
        ...
        try {
            const userName: string | undefined = await new Promise(async (resolve: (res: string | undefined) => void, reject: (e: Error) => void): Promise<void> => {
                ...
                const notionSecret = await vscode.window.showInputBox({
                    placeHolder: "Eg: secret_5Kjashdofjaiosdjfkojaskdjfkoasd",
                    prompt: "Enter your notion secret key",
                    password: true,
                    ignoreFocusOut: true
                  });
